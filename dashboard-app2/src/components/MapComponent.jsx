...
// Definisi CustomOverlay di luar komponen untuk menghindari re-render yang tidak perlu
// Ini akan memberikan akses yang lebih stabil ke proyeksi peta.
class CustomOverlay extends window.google.maps.OverlayView {
  constructor(map) {
    super();
    this.map = map;
    this.projectionReady = false; // Flag untuk menunjukkan apakah proyeksi sudah siap
    this.setMap(map); // Langsung pasang ke peta saat diinisialisasi
  }

  onAdd() {
    // Metode ini dipanggil saat overlay ditambahkan ke peta.
    // Pada titik ini, proyeksi peta sudah tersedia.
    this.projectionReady = true;
    console.log('CustomOverlay: onAdd - projection ready.');
  }

  draw() {
    // Metode ini dipanggil saat overlay perlu digambar atau digambar ulang.
    // Untuk kasus ini, kita tidak perlu menggambar elemen visual apa pun.
  }

  onRemove() {
    // Metode ini dipanggil saat overlay dilepas dari peta.
    this.projectionReady = false;
    console.log('CustomOverlay: onRemove.');
  }

  // Metode helper untuk mendapatkan proyeksi kanvas peta
...
const MapComponent = ({ atmLocations = [], getTierColor, activeView, userLocation = null }) => {
  ...
  useEffect(() => {
    ...
    if (!overlayRef.current) {
      ...
      // Jika overlay sudah ada (misalnya karena re-render), pastikan terpasang ke peta yang benar
